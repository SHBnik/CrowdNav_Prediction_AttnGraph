...
class CrowdSimPredRealGST(CrowdSimPred):
    ...
    def generate_ob(self, reset, sort=False):
        ...
        ob["spatial_edges"] = np.tile(
            parent_ob["spatial_edges"], self.predict_steps + 1
        )
    ...
    def calc_reward(self, action, danger_zone="future"):
        ...
        reward, done, episode_info = super(CrowdSimPred, self).calc_reward(
        ...
        for i in range(len(self.humans)):
            ...
            if self.gst_out_traj is not None:
                for j in range(self.predict_steps):
                    circle = plt.Circle(
                        self.gst_out_traj[i, (2 * j) : (2 * j + 2)]
                        + np.array([robotX, robotY]),
                        self.config.humans.radius,
                        fill=False,
                        color="tab:orange",
                        linewidth=1.5,
                    )
