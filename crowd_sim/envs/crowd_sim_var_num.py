...
class CrowdSimVarNum(CrowdSim):
        ...
        if self.record:
            ...
            if self.robot.kinematics == "unicycle":
                ...
                self.human_num = np.random.randint(
                    1, self.config.sim.human_num + self.human_num_range + 1
                )
            ...
            # for sim exp
                ...
                # generate humans
                self.human_num = np.random.randint(
                    low=self.config.sim.human_num - self.human_num_range,
    ...
    def calc_human_future_traj(self, method):
        ...
        for i in range(self.human_num):
            ...
            self.human_future_traj[0, i] = np.array(
                self.humans[i].get_observable_state_list()[:-1]
            )
        ...
        if self.robot.visible:
            self.human_future_traj[0, -1] = np.array(
            ...
            for i in range(1, self.buffer_len + 1):
                for j in range(self.human_num):
                    ...
                    full_state = np.concatenate(
                        (
                            self.human_future_traj[i - 1, j],
                            self.humans[j].get_full_state_list()[4:],
                        )
                    )
                    observable_states = []
                    for k in range(self.human_num):
                        ...
                        observable_states.append(
                            np.concatenate(
                                (
                                    self.human_future_traj[i - 1, k],
                                    [self.humans[k].radius],
                                )
                            )
                        )

                    # use joint states to get actions from the states in the last step (i-1)
                    action = self.humans[j].act_joint_state(
                ...
                if self.robot.visible:
                    ...
                    # update px, py, vx, vy
                    self.human_future_traj[i, -1] = self.robot.one_step_lookahead(
                        self.human_future_traj[i - 1, -1, :2], action
                    )
        ...
        # for const vel model
            ...
            self.human_future_traj = np.tile(
                self.human_future_traj[0].reshape(1, human_num, 4),
            ...
            self.human_future_traj[:, :, :2] = (
                self.human_future_traj[:, :, :2] + pred_disp
            )
        else:
            raise NotImplementedError
    ...
    def generate_ob(self, reset, sort=False):
        ...
        # nodes
        visible_humans, num_visibles, self.human_visibility = (
    ...
    def reset(self, phase="train", test_case=None):
        ...
        for i in range(self.human_num):
            self.cur_human_states[i] = np.array(
                [self.humans[i].px, self.humans[i].py, self.humans[i].radius]
            )
        ...
        rob_goal_vec = np.array([self.robot.gx, self.robot.gy]) - np.array(
    ...
    def calc_reward(self, action, danger_zone="circle"):
        ...
        if danger_zone == "circle" or self.phase == "train":
            ...
            collision_idx = (
                relative_dist < self.robot.radius + self.config.humans.radius
            )  # [predict_steps, human_num]
    ...
    def render(self, mode="human"):
        ...
        sensor_range = plt.Circle(
            ...
            self.robot.sensor_range + self.robot.radius + self.config.humans.radius,
            fill=False,
            linestyle="--",
        )
        ...
        human_circles = [
            plt.Circle(human.get_position(), human.radius, fill=False, linewidth=1.5)
        ...
        for i in range(len(self.humans)):
            ...
            plt.text(
                self.humans[i].px - 0.1,
                self.humans[i].py - 0.1,
