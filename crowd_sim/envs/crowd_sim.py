...
class CrowdSim(gym.Env):
    ...
    def smooth_action(self, action):
        ...
        if self.phase == "test":
            left = (1.0 - beta) * self.last_left + beta * left
            right = (1.0 - beta) * self.last_right + beta * right
        ...
        if left > 0:
            ...
            left = max(0.0, adjust_left)
        else:
            adjust_left = left + np.random.normal(loc=1.8, scale=0.15)
            left = min(0.0, adjust_left)

        if right > 0:
            adjust_right = right - np.random.normal(loc=1.8, scale=0.15)
            right = max(0.0, adjust_right)
        else:
            adjust_right = right + np.random.normal(loc=1.8, scale=0.15)
            right = min(0.0, adjust_right)
    ...
    def generate_ob(self, reset):
        visible_human_states, num_visible_humans, human_visibility = (
