...
class CrowdSimPred(CrowdSimVarNum):
    ...
    def set_robot(self, robot):
        ...
        d["spatial_edges"] = gym.spaces.Box(
            ...
            high=np.inf,
            shape=(
                self.config.sim.human_num + self.config.sim.human_num_range,
    ...
    def generate_ob(self, reset, sort=True):
        ...
        predicted_states = self.calc_human_future_traj(
            method=self.pred_method
        )  # [pred_steps + 1, human_num, 4]
        ...
        # [human_num, (pred_steps + 1)*2]
        ob["spatial_edges"][: self.human_num][self.human_visibility] = (
        ...
        if self.config.args.sort_humans:
            ob["spatial_edges"] = np.array(
                sorted(ob["spatial_edges"], key=lambda x: np.linalg.norm(x[:2]))
            )
    ...
    def step(self, action, update=True):
        ...
        if self.robot.policy.name == "ORCA":
            ...
            human_states = np.concatenate(
                (
                    human_states.reshape((-1, 4)),
                    np.tile(
                        self.last_human_states[:, -1], self.predict_steps + 1
                    ).reshape((-1, 1)),
                ),
                axis=1,
            )
        ...
        if self.human_num_range > 0 and self.global_time % 5 == 0:
            ...
            if np.random.rand() < 0.5:
                ...
                remove_num = np.random.randint(
                    low=0, high=min(self.human_num_range, max_remove_num) + 1
                )
    ...
    def calc_reward(self, action, danger_zone="future"):
        ...
        reward, done, episode_info = super().calc_reward(
        ...
        collision_idx = (
            ...
            < self.robot.radius + self.config.humans.radius
        ...
        reward_future = (
            collision_idx * collision_penalties
        )  # [predict_steps, human_num]
    ...
    def render(self, mode="human"):
        ...
        for i in range(len(self.humans)):
            ...
            if (
                -actual_arena_size <= self.humans[i].px <= actual_arena_size
                and -actual_arena_size <= self.humans[i].py <= actual_arena_size
            ):
