...
class SpatialEdgeSelfAttn(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.multihead_attn = torch.nn.MultiheadAttention(
            self.attn_size, self.num_attn_heads
        )
    ...
    def forward(self, inp, each_seq_len):
        ...
        if self.args.sort_humans:
            attn_mask = self.create_attn_mask(
                each_seq_len, seq_len, nenv, max_human_num
            )  # [seq_len*nenv, 1, max_human_num]
        ...
        input_emb = torch.transpose(
            input_emb, dim0=0, dim1=1
        )  # if we use pytorch builtin function, v1.7.0 has no batch first option
        q = self.q_linear(input_emb)
        k = self.k_linear(input_emb)
        v = self.v_linear(input_emb)

        # z=self.multihead_attn(q, k, v, mask=attn_mask)
        z, _ = self.multihead_attn(
            q, k, v, key_padding_mask=torch.logical_not(attn_mask)
        )  # if we use pytorch builtin function
    ...
    def forward(self, h_temporal, h_spatials, each_seq_len):
        ...
        for i in range(self.num_attention_head):
            ...
            weighted_value, attn = self.att_func(
                temporal_embed, spatial_embed, h_spatials, attn_mask=attn_mask
            )
...
class selfAttn_merge_SRNN(nn.Module):
    ...
    def __init__(self, obs_space_dict, args, infer=False):
        ...
        self.robot_linear = nn.Sequential(
            init_(nn.Linear(robot_size, 256)), nn.ReLU()
        )  # todo: check dim
        ...
        if self.args.use_self_attn:
            ...
            self.spatial_linear = nn.Sequential(
                init_(nn.Linear(obs_space_dict["spatial_edges"].shape[1], 128)),
                nn.ReLU(),
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        ...
        if self.args.sort_humans:
            ...
            human_masks = reshapeT(
                inputs["visible_masks"], seq_length, nenv
            ).float()  # [seq_len, nenv, max_human_num]
        ...
        if self.args.no_cuda:
            all_hidden_states_edge_RNNs = Variable(
                torch.zeros(
                    ...
                    nenv,
                    1 + self.human_num,
                    rnn_hxs["human_human_edge_rnn"].size()[-1],
                ).cpu()
            )
        else:
            all_hidden_states_edge_RNNs = Variable(
        ...
        if self.args.sort_humans:
            ...
            if self.args.use_self_attn:
                spatial_attn_out = self.spatial_attn(
            ...
            # human-human attention
            if self.args.use_self_attn:
                spatial_attn_out = self.spatial_attn(spatial_edges, human_masks).view(
            ...
            # robot-human attention
            hidden_attn_weighted, _ = self.attn(
                robot_states, output_spatial, human_masks
            )
